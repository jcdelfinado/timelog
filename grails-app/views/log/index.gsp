			...
			<thead>
				...
				<g:each in="${logInstanceList}" status="i" var="logInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${logInstance.id}">${fieldValue(bean: logInstance, field: "isLogIn")}</g:link></td>
					
						<td>${fieldValue(bean: logInstance, field: "employee")}</td>
					
						<td><g:formatDate date="${logInstance.logTime}" /></td>
